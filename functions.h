...
void print_client(clientinfo ci)
{
  ...
  if (ci.err != 0) {
    ...
    Serial.printf("DEVICE: ");
     ...
     for (int i = 0; i <= 6; i++) {
      char snum[3];
      Serial.printf("%d",ci.station[i]);
      Serial.printf(":");
     //itoa(ci.station[i],snum,10);
//      clientaddress+= snum;
      //clientaddress+=":";  
...
//     Serial.println(converter(ci.station));
      Serial.printf(" ==> ");
...
//          String link = "a=" + ci.station[0] + "b=" + ci.station[1] + "c=" + ci.station[2] + "d=" + ci.station[3] + "e="+ci.station[4] + "f="+ci.station[5];
//          Serial.println(link);
          print_client(ci);
          nothing_new = 0;
        }
